...
@partial(jax.vmap, in_axes=(None, None, None, 0, 0, 0, 0, 0, None, None), out_axes=0) # batch 
def inference(model, params, g, W, A, X, Y, Z, C=None, X_xrd=None):
    XYZ = jnp.concatenate([X[:, None],
                           ...
                           axis=-1)
    M = mult_table[g-1, W]  
    return model(params, None, g, XYZ, A, W, M, False, C, X_xrd)
...
def sample_crystal(key, transformer, params, n_max, batchsize, atom_types, wyck_types, Kx, Kl, g, w_mask, atom_mask, top_p, temperature, T1, constraints, C=None, X_xrd=None):
       
    def body_fn(i, state):
        key, W, A, X, Y, Z, L = state 

        # (1) W 
        w_logit = inference(transformer, params, g, W, A, X, Y, Z, C, X_xrd)[:, 5*i] # (batchsize, output_size)
        ...
        # (2) A
        h_al = inference(transformer, params, g, W, A, X, Y, Z, C, X_xrd)[:, 5*i+1] # (batchsize, output_size)
        ...
        # (3) X
        h_x = inference(transformer, params, g, W, A, X, Y, Z, C, X_xrd)[:, 5*i+2] # (batchsize, output_size)
        ...
        # (4) Y
        h_y = inference(transformer, params, g, W, A, X, Y, Z, C, X_xrd)[:, 5*i+3] # (batchsize, output_size)
        ...
        # (5) Z
        h_z = inference(transformer, params, g, W, A, X, Y, Z, C, X_xrd)[:, 5*i+4] # (batchsize, output_size)
